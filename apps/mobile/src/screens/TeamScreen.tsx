...
export function TeamScreen() {
  ...
  if (!currentUser || currentUser.role === 'customer') {
    return <Screen>
      <AppHeader title="Access unavailable" subtitle="Team permission is required" back />
      <Card><EmptyState icon="lock" title="Team is restricted" body="This area is only available to administrators and staff." /></Card>
    </Screen>;
  }
...
function TeamContent() {
  ...
  const defaultBranchId = activeBranches.some((branch) => branch.id === state.activeBranchId)
  ...
  usePreventRemove(Boolean(credentials), () => {
    Alert.alert(
      'Temporary login details are still visible',
      'Share or record these one-time credentials, then dismiss them with the close button before leaving this screen.',
  ...
  const hydrateFromAction = async (action: AppAction) => {
    ...
    dispatch({
      type: 'HYDRATE',
      state: isAdmin ? { ...remoteState, activeBranchId: selectedAdminBranchId } : remoteState,
    });
  };
  ...
  const addStaff = async () => {
    ...
    if (!activeBranches.some((branch) => branch.id === branchId)) {
      setFormError('Choose an active branch before creating this account.'); return;
    }
  ...
  return <Screen>
    <AppHeader title={isAdmin ? 'Team management' : 'Branch teammates'} subtitle={isAdmin ? 'Accounts, assignments and employment history' : 'People assigned to your branches'} back />
