import { getActiveUser, money, type AppAction, type Service } from '@gatsi/domain';
import { ArrowRight, CheckCircle2, CircleOff, Clock3, Droplets, Pencil, Scissors, Shirt, Sparkles, Truck } from 'lucide-react';
...
export function ServicesPage() {
  ...
  const saveService = async (event: React.FormEvent) => {
    ...
    if (!isAdmin || !draft || !editingServiceId || saving) return;
    ...
    const action: AppAction = { type: 'UPDATE_SERVICE', serviceId: editingServiceId, updates };
    setSaving(true);
    setFormError('');
    try {
  ...
  return <>
    <PageTitle
      ...
      actions={user.role === 'customer' ? <Link to="/pickup"><Button><Truck /> Book pickup</Button></Link> : <Link to="/orders/new"><Button>New order <ArrowRight /></Button></Link>}
    />
    ...
    {isAdmin && draft && editingServiceId ? <Card className="inline-form admin-edit-form">
      <div className="card-heading"><div><span className="eyebrow">Catalogue settings</span><h2>Edit {draft.name}</h2></div></div>
      <form className="form-grid two" onSubmit={(event) => void saveService(event)} aria-busy={saving}>
        ...
        <label className={`admin-active-toggle ${draft.active ? 'selected' : ''}`}><input type="checkbox" disabled={saving} checked={draft.active} onChange={(event) => updateDraft('active', event.target.checked)} /><span>{draft.active ? <CheckCircle2 /> : <CircleOff />}</span><div><strong>{draft.active ? 'Available service' : 'Unavailable service'}</strong><small>Unavailable services stay on past orders but cannot be selected for new work.</small></div></label>
        ...
        <div className="form-actions"><Button type="button" variant="secondary" disabled={saving} onClick={cancelEdit}>Cancel</Button><Button type="submit" disabled={saving}>{saving ? 'Saving…' : 'Save service'}</Button></div>
      </form>
    </Card> : null}
    ...
    <div className="services-grid">{services.map((service) => <Card className={`service-tile ${service.active ? '' : 'record-inactive'}`} key={service.id}>
      ...
      {isAdmin ? <div className="record-actions service-actions"><span className={`availability-label ${service.active ? '' : 'inactive'}`}>{service.active ? 'Available' : 'Unavailable'}</span><Button variant="secondary" onClick={() => beginEdit(service)}><Pencil /> Edit</Button></div> : null}
    </Card>)}</div>
  </>;
}
