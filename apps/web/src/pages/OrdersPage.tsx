...
export function OrdersPage() {
  ...
  return <><PageTitle eyebrow={user.role === 'customer' ? 'My garments' : user.role === 'staff' ? 'My work' : 'Operations'} title={user.role === 'customer' ? 'Track orders' : user.role === 'staff' ? 'My assigned orders' : 'Orders'} description={user.role === 'customer' ? 'Every care stage, payment and expected completion date.' : user.role === 'staff' ? 'Search and advance jobs assigned to your profile.' : 'Search, filter and advance every branch order.'} actions={user.role !== 'customer' ? <Link to="/orders/new"><Button><Plus /> New order</Button></Link> : undefined} /><div className="toolbar"><label className="search-box"><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search order, customer or garment..." /></label><div className="filter-label"><Filter /> Status</div></div><div className="filter-tabs">{filters.map((item) => <button key={item} className={filter === item ? 'active' : ''} onClick={() => setFilter(item)}><span>{item === 'all' ? 'All orders' : statusLabels[item]}</span><b>{item === 'all' ? orders.length : orders.filter((order) => order.status === item).length}</b></button>)}</div><div className="results-heading"><span>{results.length} matching {results.length === 1 ? 'order' : 'orders'}</span><small>Fetched from the shared database</small></div><section className="card order-list order-list-page">{results.map((order) => <OrderRow key={order.id} state={state} order={order} />)}{!results.length ? <Empty title="No matching orders" body="Try another search term or choose a different status." /> : null}</section></>;
}
