...
After one successful online sign-in, both clients keep the authenticated, role-scoped workspace available when the API or internet connection is unavailable. New customers and orders, workflow updates, staff clock-in/out events, payments, pickups, inventory adjustments, new branches/services, retail products and retail sales update the interface immediately and are stored in a FIFO queue. The web app always sends online changes immediately and replays queued work whenever connectivity returns. On mobile, administrators choose whether devices replay that queue whenever connectivity returns or only on the first online use of each Africa/Harare day; a user can always tap the synchronization indicator to synchronize immediately. The indicator shows offline, synchronized, syncing, pending and rejected states.
...
Administrators can maintain branches, services, staff, customers, their own login username and profile, the mobile synchronization schedule, and saleable clothing stock. A dedicated Store view keeps retail products separate from services and operating inventory. A customer checkout can contain multiple different products and quantities from one branch, with a separately negotiated price for every line; stock is validated and reduced atomically. Selecting a customer account makes the resulting receipt visible to that customer. Every service payment and store purchase creates a permanent, role-scoped receipt in the same atomic state update. Receipts preserve the transaction-time customer, branch, issuer, line items, list and final prices, payment method and balance; they remain available from dedicated receipt history and detail views on web and mobile. Operations reporting provides an on-demand current-day view of orders and transactions, refreshes from PostgreSQL every minute while open, and falls back to the authenticated device cache when offline. Permanent completed-day summaries remain stored separately for historical reporting.
